...
func main() {
	...
	result2 := GetNearestIntersectPart2(set1, set2)
	fmt.Println("Result: ", result2)

}

func GetNearestIntersect(set1 []string, set2 []string) int {
	s1 := ParseInputData(set1)
	s2 := ParseInputData(set2)

	intersects := FindIntersects(s1, s2)
	fmt.Println(intersects)
	res := GetNearestDistance(intersects)
	return res
}

func GetNearestIntersectPart2(set1 []string, set2 []string) int {
	s1 := ParseInputData(set1)
	s2 := ParseInputData(set2)
	fmt.Println("s1:", s1)
	fmt.Println("s2:", s2)
	intersects := FindIntersectsPart2(s1, s2)
	fmt.Println(intersects)
	res := GetNearestDistancePart2(intersects)
	return res
}

func ParseInputData(set []string) []Section {
	...
	for _, v := range set {
		...
		section := Section{A: Point{X: start.X, Y: start.Y}, B: Point{X: start.X, Y: start.Y}, PreviousLen: length}
		switch v[0] {
		case 'R':
			section.B.X += val
			start = section.B
...
func FindIntersects(set1 []Section, set2 []Section) []Point {
	result := make([]Point, 0)
	for _, s1 := range set1 {
		for _, s2 := range set2 {
			intersect, found := DetectIntersect(s1, s2)
			if found == true {
...
func FindIntersectsPart2(set1 []Section, set2 []Section) []Intersection {
	result := make([]Intersection, 0)
	for _, s1 := range set1 {
		for _, s2 := range set2 {
			intersect, found := DetectIntersect(s1, s2)
			if found == true {
				var start1, start2 Point
				if s1.StartA == true {
					start1 = s1.A
				} else {
					start1 = s1.B
				...
				fmt.Println("s1.PreviousLen: ", s1.PreviousLen)
				fmt.Println("s2.PreviousLen: ", s2.PreviousLen)
				fmt.Println("ManhattanDistance(start1, intersect): ", ManhattanDistance(start1, intersect))
				fmt.Println("ManhattanDistance(start2, intersect): ", ManhattanDistance(start2, intersect))

				is := Intersection{Point: intersect, LenSet1: s1.PreviousLen + ManhattanDistance(start1, intersect), LenSet2: s2.PreviousLen + ManhattanDistance(start2, intersect)}
...
func DetectIntersect(s1 Section, s2 Section) (Point, bool) {
	...
	if s1.A.X == s1.B.X {
		//s1 vertical
		if s2.A.X <= s1.A.X && s2.B.X >= s1.A.X && s1.A.Y <= s2.A.Y && s1.B.Y >= s2.A.Y {
			res = Point{X: s1.A.X, Y: s2.A.Y}
		}
	} else {
		//s2 horizontal
		if s1.A.X <= s2.A.X && s1.B.X >= s2.A.X && s2.A.Y <= s1.A.Y && s2.B.Y >= s1.A.Y {
			res = Point{X: s2.A.X, Y: s1.A.Y}
		}
	}
...
func GetNearestDistance(intersects []Point) int {
	minDist := 0
	for _, v := range intersects {
		dist := int(math.Abs(float64(v.X)) + math.Abs(float64(v.Y)))
...
func GetNearestDistancePart2(intersects []Intersection) int {
	minDist := 0
	for _, v := range intersects {
		dist := v.LenSet1 + v.LenSet2
		if minDist == 0 || minDist > dist {
			minDist = dist
		}
	}
