...
func CalculateOreForReaction(reactions []Reaction, result Reagent) int {
	...
	if len(reaction.Ingredients) == 1 && reaction.Ingredients[0].Name == "ORE" {
		return reactionNumber * reaction.Ingredients[0].Quantity
	} else {
		oneReactionOre := 0
		for _, v := range reaction.Ingredients {
			oneReactionOre += CalculateOreForReaction(reactions, v)
		}
		return oneReactionOre * reactionNumber
